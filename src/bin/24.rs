...
fn part1(input: &str) -> Result<usize, Box<Error>> {
    let armies = Armies::parse_groups(&input)?;
    armies.groups.iter().for_each(|g| println!("{:?}", g));
    Ok(0)
}
...
impl From<Vec<Group>> for Armies {
...
impl Armies {
    fn parse_groups(input: &str) -> Result<Self, Box<Error>> {
        lazy_static! {
